...
import com.dac.base.core.coretypes.uint.UInt64;
import com.dac.base.core.fields.AmountField;
...
/**
 * In ripple, amounts are either DAC, the native currency, or an IOU of
...
//    public static final Amount NEUTRAL_ZERO = new Amount(Currency.NEUTRAL, AccountID.NEUTRAL);
    ...
    // The smallest quantity of an DAC is a drop, 1 millionth of an DAC
    ...
    // The quantity of DAC or Issue(currency/issuer pairing)
    // When native, the value unit is DAC, not drops.
    private BigDecimal value;
    private Currency currency;
    // If the currency is DAC
    ...
    */
    ...
    /**
     * @return Amount
     * The real native unit is a drop, one million of which are an DAC.
     * We want `one` unit at DAC scale (1e6 drops), or if it's an IOU,
     * just `one`.
     */
    public Amount one() {
        if (isNative()) {
    ...
    public static class Translator extends TypeTranslator<Amount> {
        ...
        public Amount fromString(String s) {
            ...
            return com.dac.base.core.coretypes.Amount.fromString(s);
        }
    ...
    public String nativeText() {
        return String.format("%s/DAC", valueText());
    }
    ...
    /**
     * @return A String containing the value as a decimal number (in DAC scale)
     */
    ...
    private static PrecisionError getOutOfBoundsError(BigDecimal abs, String sized, BigDecimal bound) {
        return new PrecisionError(abs.toPlainString() + " absolute DAC is " + sized + bound);
    }
