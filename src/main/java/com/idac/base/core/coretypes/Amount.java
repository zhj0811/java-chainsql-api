...
import com.idac.base.core.fields.AmountField;
...
/**
 * In ripple, amounts are either IDAC, the native currency, or an IOU of
...
//    public static final Amount NEUTRAL_ZERO = new Amount(Currency.NEUTRAL, AccountID.NEUTRAL);
    ...
    // The smallest quantity of an IDAC is a drop, 1 millionth of an IDAC
    ...
    // The quantity of IDAC or Issue(currency/issuer pairing)
    // When native, the value unit is IDAC, not drops.
    private BigDecimal value;
    private Currency currency;
    // If the currency is IDAC
    ...
    */
    ...
    /**
     * @return Amount
     * The real native unit is a drop, one million of which are an IDAC.
     * We want `one` unit at IDAC scale (1e6 drops), or if it's an IOU,
     * just `one`.
     */
    public Amount one() {
        if (isNative()) {
    ...
    public static class Translator extends TypeTranslator<Amount> {
        ...
        public Amount fromString(String s) {
            ...
            return com.idac.base.core.coretypes.Amount.fromString(s);
        }
    ...
    public String nativeText() {
        return String.format("%s/IDAC", valueText());
    }
    ...
    /**
     * @return A String containing the value as a decimal number (in IDAC scale)
     */
    ...
    private static PrecisionError getOutOfBoundsError(BigDecimal abs, String sized, BigDecimal bound) {
        return new PrecisionError(abs.toPlainString() + " absolute IDAC is " + sized + bound);
    }
