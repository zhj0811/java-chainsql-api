package com.idac.base.client.payments;
...
import com.idac.base.core.coretypes.*;
...
public class PaymentFlow extends Publisher<PaymentFlow.events> {
    ...
    public void makePathFindRequestIfCan() {
        ...
        if (destAmountCurrency.equals(Currency.IDAC)) {
            // TODO: some way of ...
            destinationAmount = Issue.IDAC.amount(destAmountValue);
        } else {
            destinationAmount = new Amount(destAmountValue, destAmountCurrency, dest);
        }

        if (destinationAmount.isNative()) {
            // TODO, check if destination has no IDAC flag set ;)
    ...
    public ManagedTxn createPayment(Alternative alternative, BigDecimal sendMaxMultiplier) {
        ...
        // If we are sending IDAC directly it's pointless to specify SendMax
